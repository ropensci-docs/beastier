...
- Add ‘remove_beaustier_folders’ to delete both the ‘beautier’ and
  ‘beastier’ temporary folders
- Add ‘check_empty_beaustier_folders’ to check that both the ‘beautier’
  and ‘beastier’ temporary folders are empty
...
- `install_beast2`, `upgrade_beast2`, `uninstall_beast2` are deprecated,
  as these violated CRAN policy. Thanks to Brian Ripley for sharing! The
  deprecation message will point users to the non-official
  `beastierinstall` package at
...
- Can correctly call BEAST2 when spaces in
  ...
  4.  BEAST2 state output filename. Thanks Jason Griffiths
...
- The function arguments `beast2_output_log_filename` and
  `beast2_output_trees_filename` have become obsolete. Use
  `create_tracelog()$filename` and `create_treelog()$filename` as part
  of an MCMC instead
- The function argument `beast2_working_dir` is obsoleted, because it no
  longer served a purpose. Use `setwd` to set the working directory

## beastier 2.0.25 (2019-10-10)

### NEW FEATURES

- Add `get_beast2_example_filenames` to get the full paths of all the
  BEAST2 example files
- Add `get_beast2_example_filename` to get the full path of a BEAST2
  example file
- Add `get_default_beast2_working_dir` to get the full path of the
  default BEAST2 working directory
...
- Do not check if `beast2_path` still/already exists in
...
- Use BEAST2 main class name when calling the jar file, to fix the ‘no
...
- Lowered cyclomatic complexity, thanks
  [@lintr-bot](https://github.com/lintr-bot)
...
- RNG seed is checked to be a whole number (Issue
  [\#35](https://github.com/ropensci/beastier/issues/35), thanks
  [@thijsjanzen](https://github.com/thijsjanzen))
...
- Implemented feedback from CRAN, thanks Martina Schmirl:
  ...
  - In DESCRIPTION, use only `GPL-3` instead of also adding the file
    `LICENSE`
...
- Instead of supplying a BEAST2 `.jar` path (using the argument
  `beast2_jar_path`), one can now also supply the BEAST2 binary path.
...
- `run_beast2` had a parameter called `overwrite_state_file`, which is
  renamed to `overwrite` and states if the `.log` file and `.trees`
  files will be overwritten, like BEAST2 does. This is a bug fix, as the
  obsolete parameter name did not prevent the state file being
  overwritten.

### DEPRECATED AND DEFUNCT

- Calling `run_beast2` with a parameter called `overwrite_state_file` is
  obsolete. Use the parameter `overwrite` instead.
...
- Follow all [rOpenSci packaging
  guidelines](https://devguide.ropensci.org/pkg_building.html)
